...
const SAMPLE_PAD_DATA: &[u8] = include_bytes!("/Users/lexchristopherson/Developer/music-lang/examples/samples/kick.wav");
...
impl Default for LoopingSampler {
    fn default() -> Self {
        Self {
...
impl Plugin for LoopingSampler {
    const NAME: &'static str = "Looping Sampler";
    ...
    fn initialize(
        ...
        {
            let cursor = std::io::Cursor::new(SAMPLE_PAD_DATA);
            let reader = hound::WavReader::new(cursor).expect("invalid WAV: pad");
            let spec = reader.spec();
            self.sample_pad_rate = spec.sample_rate;
            self.sample_pad = match spec.sample_format {
        ...
        while block_start < num_samples {
            ...
            for voice in self.voices.iter_mut().filter_map(|voice| voice.as_mut()) {
                for value_idx in 0..block_len {
                    ...
                    let output_sample = if voice.loop_active_0 { let __pos = voice.loop_pos_0 as usize; if __pos < self.sample_pad.len() { let __s = self.sample_pad[__pos]; voice.loop_pos_0 += 1.0; if voice.loop_pos_0 as usize >= self.sample_pad.len() { voice.loop_pos_0 = 0.0; } __s } else { voice.loop_pos_0 = 0.0; 0.0_f32 } } else { 0.0_f32 } * voice.velocity;
...
impl LoopingSampler {
    ...
    fn start_voice(
        ...
        let new_voice = Voice {
            ...
            slide: 0.0,
            releasing: false,
            loop_pos_0: 0.0, loop_active_0: true,
        };
...
impl ClapPlugin for LoopingSampler {
    const CLAP_ID: &'static str = "dev.museaudio.looping-sampler";
    const CLAP_DESCRIPTION: Option<&'static str> = Some("A looping sample playback instrument");
...
impl Vst3Plugin for LoopingSampler {
    const VST3_CLASS_ID: [u8; 16] = *b"MuseLoopSamp01  ";
    const VST3_SUBCATEGORIES: &'static [Vst3SubCategory] = &[Vst3SubCategory::Instrument];
}

nih_export_clap!(LoopingSampler);
nih_export_vst3!(LoopingSampler);
...
mod tests {
    ...
    struct TestProcessContext {
        transport: Transport,
        events: std::collections::VecDeque<PluginNoteEvent<LoopingSampler>>,
    }
    ...
    impl ProcessContext<LoopingSampler> for TestProcessContext {
        ...
        fn next_event(&mut self) -> Option<PluginNoteEvent<LoopingSampler>> { self.events.pop_front() }
        fn send_event(&mut self, _event: PluginNoteEvent<LoopingSampler>) {}
    ...
    fn test_silence_before_notes() {
        let mut plugin = LoopingSampler::default();
        ...
        struct InitCtx;
        impl InitContext<LoopingSampler> for InitCtx {
            fn plugin_api(&self) -> PluginApi { PluginApi::Clap }
    ...
    #[test]
    fn test_loop_produces_continuous_output() {
        let mut plugin = LoopingSampler::default();
        let layout = AudioIOLayout {
            main_input_channels: NonZeroU32::new(TEST_CHANNELS as u32),
            main_output_channels: NonZeroU32::new(TEST_CHANNELS as u32),
        ...
        let mut channel_data = make_silence(44100);
        let input_rms = compute_rms(&channel_data[0]);
        let input_peak = compute_peak(&channel_data[0]);
        let mut ctx = TestProcessContext::new(TEST_SAMPLE_RATE);
        ctx.events.push_back(NoteEvent::NoteOn { timing: 0, voice_id: None, channel: 0, note: 60, velocity: 0.800000 });
        ctx.events.push_back(NoteEvent::NoteOff { timing: 44100, voice_id: None, channel: 0, note: 60, velocity: 0.0 });
        let output = run_process(&mut plugin, &mut channel_data, &mut ctx);
        ...
        if !(output_rms_db > -40.000000_f32) {
            panic!("{}", muse_test_fail("loop produces continuous output", "output.rms > -40.0 dB", "> -40.0 dB", &format!("{:.2}", output_rms_db)));
        }
    }
    ...
    fn test_no_nan() {
        ...
        let mut ctx = TestProcessContext::new(TEST_SAMPLE_RATE);
        ctx.events.push_back(NoteEvent::NoteOn { timing: 0, voice_id: None, channel: 0, note: 60, velocity: 0.800000 });
        ctx.events.push_back(NoteEvent::NoteOff { timing: 4096, voice_id: None, channel: 0, note: 60, velocity: 0.0 });
        let output = run_process(&mut plugin, &mut channel_data, &mut ctx);
...
if sample.is_nan() {
panic!("{}", muse_test_fail("no NaN", "no_nan", "no NaN values", &format!("NaN at channel {} sample {}", ch_idx, s_idx)));
}
}
}
    }
...
mod muse_preview {
    ...
    impl ProcessContext<LoopingSampler> for PreviewProcessContext {
        fn plugin_api(&self) -> PluginApi { PluginApi::Clap }
        fn execute_background(&self, _task: ()) {}
        fn execute_gui(&self, _task: ()) {}
        fn transport(&self) -> &Transport { &self.transport }
    ...
    pub unsafe extern "C" fn muse_preview_create(sample_rate: f32) -> *mut u8 {
        ...
        impl InitContext<LoopingSampler> for PreviewInitCtx {
            fn plugin_api(&self) -> PluginApi { PluginApi::Clap }
            fn execute(&self, _task: ()) {}
            fn set_latency_samples(&self, _samples: u32) {}
            fn set_current_voice_capacity(&self, _capacity: u32) {}
