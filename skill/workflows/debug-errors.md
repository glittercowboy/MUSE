...
**E003 (Unknown Function):** Check the function name against the 16 registered DSP primitives: `sine`, `saw`, `square`, `triangle`, `noise`, `lowpass`, `highpass`, `bandpass`, `notch`, `adsr`, `ar`, `gain`, `pan`, `delay`, `mix`, `clip`, `tanh`. If the compiler gives a "Did you mean?" suggestion, use it. Note: `fold` is not registered — use `tanh` instead.
